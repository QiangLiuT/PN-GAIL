...
parser.add_argument('--early-stop', type=int, default=400)
...
def update_params(batch):
    ...
    fixed_log_prob = normal_log_density(Variable(actions.cpu()), action_means, action_log_stds,
    ...
    for _ in range(args.eval_epochs):
        ...
        for _ in range(10000):  # Don't infinite loop while learning
            ...
            if done:
                break
            state = next_state
    writer.add_scalar('eval_avg_reward', avg_reward / args.eval_epochs, episode)
    return avg_reward / args.eval_epochs
...
log_dir = os.path.join(args.ofolder, args.env, algo,
                       '{}_{}-{}_{}'.format(args.num_epochs, args.traj_size, current_time, args.seed))
...
expert_traj = torch.Tensor(expert_traj)
...
for i_episode in range(args.num_epochs):
    ...
    eval_avg_reward = evaluate(i_episode)
    ...
    if i_episode == 0:
        expert_conf = torch.ones((expert_traj.shape[0], 1))
        prob = torch.ones_like(expert_conf)
        ac_mean = torch.zeros_like(prob)
        ac_std = torch.zeros_like(ac_mean)

    if (i_episode + 1) % 50 == 0 and i_episode <= args.early_stop:
        with torch.no_grad():
            ac_mean, _, ac_std = policy_net(expert_traj[:, :num_inputs])
        ac = expert_traj[:, num_inputs:].cpu()
        ac_var = ac_std ** 2
        lg_prob = -((ac - ac_mean) ** 2) / (2 * ac_var) - torch.log(ac_std) - math.log(math.sqrt(2 * math.pi))
        prob = torch.exp(lg_prob.sum(-1, keepdim=True)).to(device)
        expert_conf = ((1 / torch.sigmoid(discriminator(expert_traj.to(device))) - 1) * prob).pow(1 / (args.beta + 1))
    ...
    weighted_loss = nn.BCEWithLogitsLoss(weight=expert_pvalue.detach())
    ...
    if i_episode % args.log_interval == 0:
        writer.add_scalar('Average reward', np.mean(reward_batch), i_episode)
        writer.add_scalar('Max reward', max(reward_batch), i_episode)
        writer.add_scalar('Min reward', min(reward_batch), i_episode)
        writer.add_scalar('Loss (disc)', disc_loss.item(), i_episode)
        print('Episode {}\tAverage reward: {:.2f}\tMax reward: {:.2f}\teval_avg_reward: {:.2f}\tLoss (disc): {:.2f}'.format(
                i_episode, np.mean(reward_batch), max(reward_batch), eval_avg_reward, disc_loss.item()))
