...
for i_episode in range(args.num_epochs):
    ...
    alpha = 0.7
    loss_fake = - disc_criterion(fake, torch.ones(states.shape[0], 1).to(device)) * alpha  #  E_{pi}[log(1-D)] * alpha  with D = sigmoid

    loss_real = - disc_criterion(real, torch.zeros(expert_state_action.size(0), 1).to(device))  #  E_{ex}[log D]

    loss_fake_real_label = - disc_criterion(fake, torch.zeros(states.shape[0], 1).to(device)) * alpha  #  E_{pi}[log D] * alpha

    threshold = loss_real.detach() - loss_fake_real_label.detach()

    if threshold <= 0:
        disc_loss = - (loss_fake + loss_real - loss_fake_real_label)
    else:
        disc_loss = - loss_fake
    disc_loss.backward()
    disc_optimizer.step()
