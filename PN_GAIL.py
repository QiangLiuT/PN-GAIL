...
log_dir = os.path.join(args.ofolder, args.env, algo,
                       '{}_{}-{}_{}_prior_{}'.format(args.num_epochs, args.traj_size, current_time, args.seed, args.prior))
...
if args.prior < 1:
    ...
    cu_loss = CULoss(beta=1 - args.prior, non=True)
...
for i_episode in range(args.num_epochs):
    ...
    expert_pvalue_neg = torch.Tensor((1 - expert_pvalue)).to(device)
    ...
    weighted_loss_neg = nn.BCEWithLogitsLoss(weight=expert_pvalue_neg)
    disc_loss = disc_criterion(fake, torch.ones(states.shape[0], 1).to(device)) + \
                ...
                weighted_loss_neg(real, torch.ones(expert_state_action.size(0), 1).to(device))
    ...
    if i_episode % args.log_interval == 0:
        ...
        print('Episode {}\tAverage reward: {:.2f}\tMax reward: {:.2f}\teval_avg_reward: {:.2f}\tLoss (disc): {:.2f}'.format(i_episode, np.mean(
            reward_batch), max(reward_batch), eval_avg_reward, disc_loss.item()))
